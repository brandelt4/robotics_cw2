...
class ReactivePlannerController(PlannerControllerBase):
    ...
    def checkIfPathCurrentPathIsStillGood(self):
        ...
        reached = False
        for idx, waypoint in enumerate(reversed(self.currentPlannedPath.waypoints)):
            # If the cell is occuppied, find a new path
            if self.occupancyGrid.getCell(waypoint.coords[0], waypoint.coords[1]) == 1.0:
                ...
                if self.method == 'basic':
                    ...
                    # ----------------------------------------------
                    # ATTEMPT to use a better local obstacle planner
                    # ----------------------------------------------
                    ...
                    #     start = (pose.x, pose.y)
                    #     # with open('/home/ros_user/catkin_ws/src/comp0037/log.txt', 'a+') as file:
                    #     #     file.write('Current position: {}'.format(self.occupancyGrid.getCellCoordinatesFromWorldCoordinates(start)))
                    #     currentCell = self.occupancyGrid.getCellCoordinatesFromWorldCoordinates(start)
